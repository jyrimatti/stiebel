...
login() {
    flock "/tmp/stiebel-$USER/lock" curl --silent --show-error --connect-timeout 30 -d "make=send&user=$STIEBEL_USER&pass=$STIEBEL_PASSWORD" --dump-header "/tmp/stiebel-$USER/headers" http://$STIEBEL_HOST/?s=0 > /dev/null
}
