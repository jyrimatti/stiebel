...
login() {
    ...
    flock "/tmp/stiebel-$USER/lock-login" curl -4 --silent --show-error --connect-timeout 30 -d "make=send&user=$STIEBEL_USER&pass=$STIEBEL_PASSWORD" --cookie-jar "/tmp/stiebel-$USER/cookies" http://$STIEBEL_HOST/?s=0 > /dev/null
}
